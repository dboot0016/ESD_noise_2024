...
# Script to plot Figure A3 for 'Observation based temperature and freshwater noise over the Atlantic Ocean'
...
#%% Manipulate data before analysis
...
def cluster_fit(nr):
    ...
    bin_mid2 = np.arange(-10,10.001,0.01)
...
for cluster_i in range(12):
    ...
    plt.plot(BIN2[cluster_i,:],NIG[cluster_i,:],color='tab:red',linewidth = LW)
    plt.plot(BIN2[cluster_i,:],NOR[cluster_i,:],color='tab:blue',linewidth = LW)
    plt.bar(BIN[cluster_i,:],HIST[cluster_i,:],color='tab:olive',width = W[cluster_i])
    
    plt.xlabel('T$_{2m}$ [$^{\circ}$C]',fontsize=FS-2)
    ...
    plt.savefig(dir_fig+'Figure_A3'+subf[cluster_i]+'.png', format='png', dpi=quality,bbox_inches='tight')
