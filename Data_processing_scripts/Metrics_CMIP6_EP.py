...
# Script for determining metrics for CMIP6 models (E - P) for 'Observation based temperature and freshwater noise over the Atlantic Ocean'
...
def determine_noise(key):
    ...
    P_E = var['e-p'][:,:,:]*A_mask_cmip*conv_int_cmip # Mask Atlantic and convert from kg m-2 s-1 to mm day-1
    
    PE = P_E-P_E.rolling(time=60, center=True,min_periods=12).mean().compute() # Detrend by subtracting 5 year moving mean
    noise = PE.groupby('time.month')-PE.groupby('time.month').mean('time').compute() # Deseasonalize
...
ser_d.to_csv(dir_cmip1+'metrics_data_ep_1deg.csv', index=True)
ser_n.to_csv(dir_cmip1+'metrics_noise_ep_1deg.csv', index=True)
