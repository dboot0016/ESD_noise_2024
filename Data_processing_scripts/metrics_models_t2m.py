...
# Uses data from 'Fit_distributions_T2m.py'
...
T2m_era5 = var['t2m'][:900,:,:]*A_mask_era*1-273

T2m = T2m_era5-T2m_era5.rolling(time=60, center=True,min_periods=12).mean().compute()
noise_era5 = T2m.groupby('time.month')-T2m.groupby('time.month').mean('time').compute()
...
data_mean_e5 = T2m_era5.mean('time')
data_std_e5 = T2m_era5.std('time')
data_skw_e5 = T2m_era5.reduce(func=scipy.stats.skew,dim='time')
data_kur_e5 = T2m_era5.reduce(func=scipy.stats.kurtosis,dim='time')
...
for key_i in range(len(methods)):
    ...
    noise_std = np.load(dir_test+'std_'+method+'_t2m.npy') * xr.ones_like(noise_mean_e5)
    noise_skw = np.load(dir_test+'skw_'+method+'_t2m.npy') * xr.ones_like(noise_mean_e5)
    noise_kur = np.load(dir_test+'kur_'+method+'_t2m.npy') * xr.ones_like(noise_mean_e5)
...
ser_n.to_csv(dir_test+'metrics_noise_t2m_noise_models.csv', index=True)
